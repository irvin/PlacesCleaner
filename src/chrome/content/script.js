...
var PlacesCleaner = {
    ...
    cleanIt: function() {
        ...
        var listener = {
            observe: function(subject, topic, data) {
                if (topic == "alertclickcallback") prompts.alert(null, "PlacesCleaner", data);
            }
        }
        ...
        if (booOnlyVacuum == false) {
            Components.classes["@mozilla.org/browser/nav-history-service;1"].getService(Components.interfaces.nsPIPlacesDatabase).DBConnection.executeSimpleSQL("DELETE FROM moz_historyvisits WHERE place_id IN (SELECT id FROM moz_places WHERE (visit_count <=" + intViewTime + ") AND (last_visit_date < " + intPreservePRtime + "));");
            Components.classes["@mozilla.org/browser/nav-history-service;1"].getService(Components.interfaces.nsPIPlacesDatabase).DBConnection.executeSimpleSQL("DELETE FROM moz_places WHERE (visit_count <= " + intViewTime + " AND (last_visit_date < " + intPreservePRtime + ") AND hidden <> 1 AND id NOT IN (SELECT place_id FROM moz_annos UNION SELECT fk FROM moz_bookmarks));");
        ...
        if (booHideStatus == true) document.getElementById("PlacesCleaner-panel").hidden = true;
    },

    getintViewTime: function() {
        return Components.classes["@mozilla.org/preferences-service;1"].getService(Components.interfaces.nsIPrefService).getBranch("extensions.PlacesCleaner.").getIntPref("viewtime");
    },

    getintDayInterval: function() {
        return Components.classes["@mozilla.org/preferences-service;1"].getService(Components.interfaces.nsIPrefService).getBranch("extensions.PlacesCleaner.").getIntPref("dayinterval");
    },

    getintPreserveDay: function() {
        return Components.classes["@mozilla.org/preferences-service;1"].getService(Components.interfaces.nsIPrefService).getBranch("extensions.PlacesCleaner.").getIntPref("preserveday");
    },

    checkAutoClean: function() {
        return Components.classes["@mozilla.org/preferences-service;1"].getService(Components.interfaces.nsIPrefService).getBranch("extensions.PlacesCleaner.").getBoolPref("autoclean");
    },

    checkOnlyVacuum: function() {
        return Components.classes["@mozilla.org/preferences-service;1"].getService(Components.interfaces.nsIPrefService).getBranch("extensions.PlacesCleaner.").getBoolPref("onlyvacuum");
    },

    checkHideStatus: function() {
        return Components.classes["@mozilla.org/preferences-service;1"].getService(Components.interfaces.nsIPrefService).getBranch("extensions.PlacesCleaner.").getBoolPref("hidestatus");
    },

    checkBackupFile: function() {
        return Components.classes["@mozilla.org/preferences-service;1"].getService(Components.interfaces.nsIPrefService).getBranch("extensions.PlacesCleaner.").getBoolPref("backupfile");
    },

    getLastVacuumTime: function() {
        return Components.classes["@mozilla.org/preferences-service;1"].getService(Components.interfaces.nsIPrefService).getBranch("extensions.PlacesCleaner.").getCharPref("lastvacuumtime");
    },
    ...
    */
    ...
    checkVerFx22NotiBug: function() {
        ...
        if (versionChecker.compare(Application.version, "22.0") >= 0) return true;
        else return false;
    }
